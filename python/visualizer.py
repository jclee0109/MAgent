...
class CustomVisualizer():
    ...
    def reset(self, pos1, pos2, ids):
        ...
        self.ax.axis('off')
        plt.tight_layout()
        fname = f'visualize/{self.current_episode}_{self.current_env_t}.png'
    ...
    #         writer = imageio.get_writer(f'visualize/video_{self.current_episode}.mp4', fps=10, format='ffmpeg')
    ...
    def make_video(self):
        if len(self.fnames) > 0:
            with imageio.get_writer(f'visualize/video_{self.current_episode}.gif', mode='I', fps=10) as writer:
                for filename in self.fnames:
